...
Nothing shares mutable state without a clear owner: the audio buffer belongs to the callback thread alone, the recording state machine is protected by a single lock, and a small `Status` object is the only thing multiple threads touch concurrently for UI state. The clipboard is the one piece three separate threads all reach (the transcriber, the thread a repaste click spawns, and the tray's re-copy), so it's serialized behind its own lock: a second paste starting mid-sequence would otherwise save the first one's text as the "prior" clipboard and hand that back instead of what was really there.
...
- Model: `large-v3-turbo` on CUDA — [SETUP.md](docs/SETUP.md) has the reasoning behind that choice.
...
[SETUP.md](docs/SETUP.md) is the actual how-to: clone, venv, model download,
