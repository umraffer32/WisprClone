...
I ran qwen2.5 at 7B for a while after that, then downsized to the 3B version once polish turned out to be the real latency cost in the whole pipeline, not whisper. A replay of the current corpus showed 3B actually no-opping less than 7B did (13% vs 31%) at roughly half the latency, with a small uptick in flagged edits that the guards above already catch and fall back on — worth it for a personal tool where an occasional missed polish costs nothing. It's still a real model, so it needs some VRAM or RAM alongside whatever whisper is already using, just meaningfully less than the 7B version did.
